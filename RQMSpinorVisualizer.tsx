...
export default function RQMSpinorVisualizer() {
  ...
  const [manualQuat, setManualQuat] = useState<number[]>([1, 0, 0, 0]); // Identity quaternion
  const [gateTape, setGateTape] = useState<QuantumGate[]>([]);
  ...
  useEffect(() => {
    document.title = "Quaternionic Quantum Computing | RQM Technologies";
    
    const metaDescription = document.createElement("meta");
    metaDescription.name = "description";
    metaDescription.content = "Interactive visualization of quaternionic quantum computing using q(t) = cos φ + u sin φ showing deterministic quaternionic rotations on the Bloch sphere with 4π periodicity and quaternionic wavefunction generalization.";
  ...
  // ============================================================================
  // UNIFIED SPINOR AND BLOCH VECTOR COMPUTATIONS
  // 
  // STANDARD SU(2) QUATERNION CONVENTION (half-angle):
  // The quaternion q = [w, x, y, z] = [cos(θ/2), uᵢ·sin(θ/2), uⱼ·sin(θ/2), uₖ·sin(θ/2)]
  // represents a rotation by angle θ about axis û = (uᵢ, uⱼ, uₖ).
  // 
  // The SU(2) spinor |ψ⟩ = U|0⟩ where U is the rotation operator:
  //   U = cos(θ/2)·I - i·sin(θ/2)·(uᵢσₓ + uⱼσᵧ + uₖσᵤ)
  // 
  // Applied to |0⟩ = (1, 0)ᵀ gives spinor coefficients directly from quaternion:
  //   α = cos(θ/2) - i·uₖ·sin(θ/2) = w - i·z
  //   β = (-uⱼ - i·uᵢ)·sin(θ/2) = -y - i·x
  // 
  // The Bloch vector is derived from the spinor coefficients:
  //   bx = 2·Re(α*·β), by = 2·Im(α*·β), bz = |α|² - |β|²
  // ============================================================================

  // Convert quaternion to spinor coefficients (α, β) as complex numbers
  // Assumes STANDARD SU(2) half-angle quaternion: q = [cos(θ/2), u·sin(θ/2)]
  // The SU(2) spinor |ψ⟩ = U|0⟩ where U = cos(θ/2)·I - i·sin(θ/2)·(û·σ)
  // Returns { alphaReal, alphaImag, betaReal, betaImag }
  const quatToSpinor = (q: number[]): { alphaReal: number; alphaImag: number; betaReal: number; betaImag: number } => {
    // q = [w, x, y, z] = [cos(θ/2), uᵢ·sin(θ/2), uⱼ·sin(θ/2), uₖ·sin(θ/2)]
    const [w, x, y, z] = q;
    
    // For half-angle quaternion, the spinor formula simplifies to:
    // α = cos(θ/2) - i·uₖ·sin(θ/2) = w - i·z
    // β = (-uⱼ - i·uᵢ)·sin(θ/2) = -y - i·x
    return {
      alphaReal: w,
  ...
  // Compute Bloch vector from spinor coefficients
  // bx = 2·Re(α*·β), by = 2·Im(α*·β), bz = |α|² - |β|²
  const spinorToBloch = (spinor: { alphaReal: number; alphaImag: number; betaReal: number; betaImag: number }): { bx: number; by: number; bz: number } => {
    const { alphaReal: ar, alphaImag: ai, betaReal: br, betaImag: bi } = spinor;
    
    // α* · β = (ar - i·ai)(br + i·bi) = (ar·br + ai·bi) + i(ar·bi - ai·br)
    const bx = 2 * (ar * br + ai * bi);  // 2·Re(α*·β)
    const by = 2 * (ar * bi - ai * br);  // 2·Im(α*·β)
    const bz = (ar * ar + ai * ai) - (br * br + bi * bi);  // |α|² - |β|²
    
    return { bx, by, bz };
  };

  // Convenience: directly compute Bloch vector from quaternion
  const quatToBloch = (q: number[]): { bx: number; by: number; bz: number } => {
  ...
  const spinors: SpinorConfig[] = customCircuitRunning && customCircuit.length > 0
    ? customCircuit.map((gateName, idx) => {
  ...
  const baseGates: Record<string, QuantumGate & { description: string }> = {
    X: { label: "X", axis: { x: 1, y: 0, z: 0 }, theta: Math.PI, description: "π around i" },
    Y: { label: "Y", axis: { x: 0, y: 1, z: 0 }, theta: Math.PI, description: "π around j" },
    Z: { label: "Z", axis: { x: 0, y: 0, z: 1 }, theta: Math.PI, description: "π around k" },
    H: { label: "H", axis: { x: 0.707107, y: 0, z: 0.707107 }, theta: Math.PI, description: "Hadamard" }, // normalized(1,0,1)
    S: { label: "S", axis: { x: 0, y: 0, z: 1 }, theta: Math.PI / 2, description: "π/2 phase" },
    T: { label: "T", axis: { x: 0, y: 0, z: 1 }, theta: Math.PI / 4, description: "π/4 phase" },
  };
  ...
  const quatConj = (q: number[]): number[] => {
    return [q[0], -q[1], -q[2], -q[3]];
  };

  // Extract axis and angle from a quaternion
  const quatToAxisAngle = (q: number[]): { axis: { x: number; y: number; z: number }; angle: number } => {
    // For q = [w, x, y, z] = cos(θ/2) + (xi + yj + zk)sin(θ/2)
    const w = Math.max(-1, Math.min(1, q[0])); // Clamp for numerical stability
    const angle = 2 * Math.acos(w);
    
    const sinHalfAngle = Math.sqrt(1 - w * w);
    
    if (sinHalfAngle < 0.001) {
      // Very small rotation, return arbitrary axis
  ...
  // Compute the gate that transforms from state1 to state2
  // Returns gate info formatted as q̇ = ½qΩ where Ω = 2φû
  const computeTransitionGate = (state1: SpinorConfig, state2: SpinorConfig): { gate: string; angle: number; axis: string; omega: string } => {
    // Create quaternions from states using half-angle SU(2) convention
    ...
    const q1 = makeQuat(state1);
    const q2 = makeQuat(state2);
    
    // Transition quaternion: q_trans = q2 * q1^(-1)
    const q1inv = quatConj(q1);
    const qTrans = quatMul(q2, q1inv);
    
    // Extract axis and angle
    const { axis, angle } = quatToAxisAngle(qTrans);
    
    // Compute Ω = 2φû for the transition
    const phi = angle;
    const omegaVec = { x: 2 * phi * axis.x, y: 2 * phi * axis.y, z: 2 * phi * axis.z };
    
    // Format Ω components for display
    const formatComponent = (val: number, unit: string) => {
      if (Math.abs(val) < 0.01) return '';
      const sign = val > 0 ? '+' : '';
      return `${sign}${val.toFixed(2)}${unit}`;
    };
    
    const iComp = formatComponent(omegaVec.x, 'i');
    const jComp = formatComponent(omegaVec.y, 'j');
    const kComp = formatComponent(omegaVec.z, 'k');
    const omegaStr = `${iComp}${jComp}${kComp}`.replace(/^\+/, '') || '0'; // Remove leading +
    
    // Axis string for display
    const axisIComp = formatComponent(axis.x, 'i');
    const axisJComp = formatComponent(axis.y, 'j');
    const axisKComp = formatComponent(axis.z, 'k');
    const axisStr = `${axisIComp}${axisJComp}${axisKComp}`.replace(/^\+/, ''); // Remove leading +
    
    const angleDeg = angle * 180 / Math.PI;
    
    return {
      gate: state2.name, // Use the gate name directly
      angle: angleDeg,
      axis: axisStr,
      omega: omegaStr
    };
  ...
  // Convert gate to quaternion: q_gate = cos(θ/2) + axis * sin(θ/2)
  const gateToQuaternion = (gate: QuantumGate): number[] => {
    const halfTheta = gate.theta / 2;
    const w = Math.cos(halfTheta);
    const s = Math.sin(halfTheta);
    return [w, gate.axis.x * s, gate.axis.y * s, gate.axis.z * s];
  };

  // Apply gate with animation
  const applyGate = (gate: QuantumGate) => {
    if (isApplyingGate) return;
    
    // Compute starting quaternion synchronously
    let qStart: number[];
    if (!manualMode) {
      // First gate application - start from current auto-cycling state
      ...
      const currentQuat = makeQuat(currentState);
      const nextQuat = makeQuat(nextState);
      
      // Use interpolated quaternion if currently animating
      qStart = isAnimating 
        ? quatSlerp(currentQuat, nextQuat, transitionProgress)
        : currentQuat;
      
      setManualMode(true);
      setManualQuat(qStart);
    } else {
      // Already in manual mode - use current manual quaternion
      qStart = [...manualQuat];
    }
    
    const qGate = gateToQuaternion(gate);
    const qEnd = quatMul(qGate, qStart); // Left multiply: q_new = q_gate * q_old
    ...
    function step(now: number) {
      ...
      const qCurrent = quatSlerp(qStart, qEnd, u);
      setManualQuat(qCurrent);
      ...
      if (u < 1) {
        ...
        setGateTape(prev => [...prev, gate]);
      }
    }
    requestAnimationFrame(step);
  };

  // Reset to auto-cycling mode
  const resetToAuto = () => {
    setManualMode(false);
    setManualQuat([1, 0, 0, 0]);
    setGateTape([]);
  ...
  const renderBlochSphere = () => {
    ...
    const projectBloch = (p: [number, number, number]): { x: number; y: number } => {
      ...
      const x = p[0];
      const y = p[1] * cosTilt - p[2] * sinTilt;
    ...
    const xAxisNeg = projectBloch([-axisLen, 0, 0]);
    const yAxisPos = projectBloch([0, axisLen, 0]);
    const yAxisNeg = projectBloch([0, -axisLen, 0]);
  ...
  const renderVisualization = () => {
    ...
    // For display purposes, extract phi from the interpolated quaternion
    const w = spinorQuat[0];
    const phi = Math.acos(Math.max(-1, Math.min(1, w))); // clamp for numerical stability
    const s = isAnimating ? currentState : currentState; // Color stays with current state

    const axesLength = 1.4;
    const xAxisRef = [project3D([axesLength, 0, 0]), project3D([-axesLength, 0, 0])];
    const yAxisRef = [project3D([0, axesLength, 0]), project3D([0, -axesLength, 0])];
    const zAxisRef = [project3D([0, 0, axesLength]), project3D([0, 0, -axesLength])];
    ...
    // Scale arrow length by interpolated phi
    const arrowLength = arrowPhi * 1.5; // Scale factor to keep visible
    const rotAxisStart = project3D([0, 0, 0]);
    const rotAxisEnd = project3D([u.x * arrowLength, u.y * arrowLength, u.z * arrowLength]);
  ...
  const nextS = spinors[(currentStateIndex + 1) % spinors.length];
  const currentPhi = s.phi0;

  return (
    <div className="min-h-screen w-full overflow-x-hidden" style={{
      background: 'linear-gradient(to bottom, #0a2533, #0b0e13)',
      minWidth: '100vw'
    }}>
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8">
        <Link href="/quaternionic-spectral-geometry" className="inline-flex items-center gap-2 text-cyan-300 hover:text-cyan-200 transition-colors mb-4 md:mb-6" data-testid="link-back">
              ...
              <div className="overflow-x-auto">
                <svg width={Math.min(1000, displaySpinors.length * 95 + 200)} height="135" className="mx-auto" style={{ minWidth: Math.min(750, displaySpinors.length * 95 + 200) }}>
                  ...
                  {displaySpinors.map((spinor, idx) => {
                    ...
                    // Compute Ω = 2φû for this gate
                    const omega = {
                      x: 2 * spinor.phi0 * spinor.u.x,
                      y: 2 * spinor.phi0 * spinor.u.y,
                      z: 2 * spinor.phi0 * spinor.u.z
                    };
                    
                    // Format Ω components
                    const formatOmegaComp = (val: number, unit: string) => {
                      if (Math.abs(val) < 0.01) return '';
                      const sign = val > 0 ? '+' : '';
                      return `${sign}${val.toFixed(1)}${unit}`;
                    };
                    const omegaStr = `${formatOmegaComp(omega.x, 'i')}${formatOmegaComp(omega.y, 'j')}${formatOmegaComp(omega.z, 'k')}`.replace(/^\+/, '') || '0';
                ...
                <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-600">
                  ...
                  <div className="border-t border-slate-600 pt-3">
                    ...
                    {(() => {
                      ...
                      // Use the actual spinor coefficients from the quaternion
                      // α = αr + i·αi, β = βr + i·βi
                      const formatComplex = (real: number, imag: number) => {
                        if (Math.abs(imag) < 0.001) return `${real.toFixed(3)}`;
                        const sign = imag >= 0 ? '+' : '';
